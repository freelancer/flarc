...
/**
 * An XHPAST linter rule for enforcing naming conventions.
 *
 * NOTE: This linter rule is heavily based on
 * `ArcanistNamingConventionsXHPASTLinterRule`. As such, you should only enable
 * exact one of these linter rules.
 *
 * @todo Submit this class upstream (as improvements to
 * `ArcanistNamingConventionsXHPASTLinterRule`) after T27678.
 */
  ...
  public function process(XHPASTNode $root) {
    ...
    foreach ($defs as $def) {
      ...
      foreach ($vars as $token_id => $var) {
        ...
        if (isset($visited_local_vars[$var_string])) {
          continue;
        }
